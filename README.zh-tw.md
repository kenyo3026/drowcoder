...
🤖 Drowcoder 是一個專門從事程式設計的通用型代理 AI Agent CLI
...
Drowcoder 是一個不依賴現有框架，而是逐步摸索與實作 AI agent 的專案，目的是逐步深入了解整個 AI agent 技術演進與堆疊——從底層基礎到應用層面，去蕪存需進而構建出一個目的最單純、方向最明確的 AI agent 框架。
